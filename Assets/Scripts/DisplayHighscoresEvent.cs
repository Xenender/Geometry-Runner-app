...
public class DisplayHighscoresEvent : MonoBehaviour 
{
    public TMPro.TextMeshProUGUI[] rNames;
    public TMPro.TextMeshProUGUI[] rScores;
    HighScoresEvent myScores;
    ...
    void Start() //Fetches the Data at the beginning
    {
        ...
        myScores = GetComponent<HighScoresEvent>();
        StartCoroutine("RefreshHighscores");
    }
    public void SetScoresToMenu(PlayerScoreEvent[] highscoreList) //Assigns proper name and score for each text value
    {
        for (int i = 0; i < rNames.Length;i ++)
        {
            rNames[i].text = i + 1 + ". ";
